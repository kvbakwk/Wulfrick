...
const VerificationManager = (config) => {
  const verifyUser = (c, gMember) => {
    ...
    const embed = new EmbedBuilder()
      .setAuthor({
        name: content.author.replace("{0}", gMember.user.username),
  ...
  const deleteVerify = (c, gMember) => {
    c.channels.cache.get("502452827562442757").messages.cache.filter((msg) => {
      if (
        msg.embeds.length &&
        msg.embeds[0].data.author.icon_url.includes(gMember.id)
      ) {
  ...
  const verifyClick = (c, i) => {
    if (i.isButton() && i.customId.length == 18) {
      if (i.customId == i.member.user.id) {
        ...
        i.member.roles.remove(
          c.guilds.cache
            .get("489087056241229845")
            .roles.cache.get("501127326453465088")
        );
        i.member.roles.add(
          c.guilds.cache
            .get("489087056241229845")
            .roles.cache.get("513438458161922079")
        );
        i.message.delete().catch(console.error);

        c.guilds.cache
          .get(i.guildId)
          .channels.cache.get(i.channelId)
          .messages.cache.filter((msg) => {
